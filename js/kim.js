...
function filterNumberMoeda(event) {
...
function loadList(jsonLs) {
  ...
  for (i = 0; i < jsonLs["Lista"].length; i++) {
    ...
    var item = `
        ...
        <span>${jsonLs["Lista"][i].Qtde} x ${parseFloat(jsonLs["Lista"][i].Valor).toLocaleString("pt-br", {
      style: "currency",
      currency: "BRL",
    })}</span>
        <span>${parseFloat(jsonLs["Lista"][i].Total).toLocaleString("pt-br", {
          style: "currency",
          currency: "BRL",
        })}</span>
...
<body>
    ...
    <ul>
        ...
        <li style="display: flex; justify-content: right; font-weight: bold; padding: 5px;">Local da Compra: ${
          itens[2]
        }</li>
    ...
    html2pdf().from(imprimir).set(options).save();
